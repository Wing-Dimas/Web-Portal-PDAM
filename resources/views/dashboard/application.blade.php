    ...
    @slot('content')
        <header class="flex justify-between">
            ...
            <div class="flex items-center">
                <button
                    class="btn-tambah bg-sea py-2 px-4 rounded-md font-semibold text-white text-xs flex items-center gap-2"><i
                        class='bx bx-plus'></i> New Application</button>
            </div>
        </header>
        ...
        <div class="relative min-h-[70vh] w-full mt-8 bg-slate-100 rounded-md py-4 px-4">
            @if ($applications->isEmpty())
                <div class="flex justify-center items-center w-full h-full absolute">
                    <span>Data empty</span>
                </div>
            @else
                <div class="cards grid grid-cols-2 gap-2">
                    @foreach ($applications as $application)
                        <div class="flex bg-white rounded-lg shadow-md p-2 gap-2">
                            <div>
                                <img src="{{ asset("/storage/$application->icon") }}" alt=""
                            ...
                            <div class="flex flex-1 flex-col">
                                <div class="relative flex justify-between">
                                    <h3 class="text-sm font-bold flex items-center gap-2">
                                        ...
                                        <span class="text-xs text-slate-300">{{ $application->group->name }}</span>
                                    </h3>
                                    <div class="flex gap-2">
                                        <button data-id="{{ $application->id }}"
                                            class="btn-edit h-min bg-orange-800 rounded-md text-white flex items-center justify-center hover:bg-orange-300 p-[2px]">
                                        ...
                                        <form action="{{ route('application.destroy', $application->id) }}" method="POST">
                                            ...
                                            @method('delete')
                                            <button type="submit"
                                                class="show-confirm bg-red-600 rounded-md text-white flex items-center justify-center hover:bg-red-400 p-[2px]"><i
                                ...
                                <div class="h-full flex flex-col justify-end">
                                    <a href="{{ $application->link }}"
                                        class="text-xs text-sea font-light w-max bg-slate-200 rounded-md px-1"
                                        target="blank">Visit link</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    @slot('modal')
        <div class="overlay absolute flex justify-center items-center top-0 left-0 right-0 bottom-0 invisible">
            ...
            <div class="modal-content relative z-50 bg-white w-96 rounded-lg shadow-lg">
                ...
                <form id="modal-form" action="{{ route('application.store') }}" method="POST"
                    ...
                    <div class="modal-main p-4">
                        <div class="grid gap-2">
                            ...
                            <input type="text" id="name" name="name"
                                class="text-xs p-2 border-slate-200 border-2 outline-none rounded-md focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                                required>
                        </div>
                        <div class="grid gap-2 mt-2">
                            <label for="description" class="font-semibold text-sm">Description</label>
                            <textarea name="description" id="description" cols="30" rows="3"
                                class="text-xs p-2 border-slate-200 border-2 outline-none rounded-md focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none resize-none"
                                required></textarea>
                        </div>
                        <div class="grid gap-2 mt-2">
                        ...
                        <div class="grid gap-2 mt-2">
                            ...
                            <input type="url" id="link" name="link"
                                class="text-xs p-2 border-slate-200 border-2 outline-none rounded-md focus:bg-white focus:border-blue-600 focus:outline-none"
                                required>
                        </div>
                        <div class="grid gap-2 mt-2">
                    ...
                    <div class="modal-footer flex flex-row-reverse py-2 px-4 border-t-2 border-slate-300">
                        <button
                            class="modal-btn bg-sea py-2 px-4 rounded-md font-semibold text-white text-xs flex items-center gap-2">Add
                            Application</button>
                    </div>
                </form>
            </div>
