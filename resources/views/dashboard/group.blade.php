            ...
            <div class="flex items-center">
                <button
                    class="btn-tambah bg-sea py-2 px-4 rounded-md font-semibold text-white text-xs flex items-center gap-2"><i
                        class='bx bx-plus'></i> New Group</button>
            </div>
        </header>
        ...
        <div class="relative min-h-[70vh] w-full mt-8 bg-slate-100 rounded-md py-4 px-4">
            <table class="table table-auto border">
                ...
                <tbody>
                    @if ($groups->isEmpty())
                        <tr>
                            <td colspan="3" class="text-center p-2 border-b">Data empty</td>
                        </tr>
                    @else
                            ...
                            <tr class="odd:bg-slate-300">
                                ...
                                <td class="py-2 px-4 border-b text-left flex gap-2">
                                    <button data-id="{{ $group->id }}"
                                        class="btn-edit bg-orange-400 rounded-md text-white p-2 flex items-center justify-center hover:bg-orange-300">
                                    ...
                                    <form action="{{ route('group.destroy', $group->id) }}" method="POST">
                                        ...
                                        @method('delete')
                                        <button type="submit"
                                            class="show-confirm bg-red-600 rounded-md text-white p-2 flex items-center justify-center hover:bg-red-400"><i
            ...
            <div class="modal-content relative z-50 bg-white w-96 rounded-lg shadow-lg">
                ...
                <form id="modal-form" action="{{ route('group.store') }}" method="POST">
                    ...
                    <div class="modal-footer flex flex-row-reverse py-2 px-4 border-t-2 border-slate-300">
                        <button
                            class="modal-btn bg-sea py-2 px-4 rounded-md font-semibold text-white text-xs flex items-center gap-2">New
                            Group</button>
                    </div>
                </form>
            </div>
